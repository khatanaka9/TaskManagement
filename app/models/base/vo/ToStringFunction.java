...
public abstract class ToStringFunction {
	...
	public static <T extends AbstractValueObject> String toString(final T vo) {
		...
		return valueField != null ? toStringByValueField(vo, valueField)
				: toStringByReflection(vo);
	}

	/** valueフィールド値を返却 */
	static <T extends AbstractValueObject> String toStringByValueField(
			final T vo, final Field field) {
		try {
	...
	/** リフレクションでフィールド値を返却 */
	private static <T extends AbstractValueObject> String toStringByReflection(
			final T vo) {
